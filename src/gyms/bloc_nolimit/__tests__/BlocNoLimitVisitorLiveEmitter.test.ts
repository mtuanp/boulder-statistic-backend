...
import { BlocNoLimitVisitorLiveEmitter } from "../BlocNoLimitVisitorLiveEmitter.ts";
...
Deno.test("testing emitter", () => {
  ...
  const db: VisitorDatastore = {
    insertVisitor: async (gym, entry) => {
      assertEquals(gym, Gym.BLOC_NO_LIMIT_CLIMBING);
  ...
  const emitter = new BlocNoLimitVisitorLiveEmitter(db, event, parser);
